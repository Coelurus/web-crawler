...
export default function RecordsHeader({
  ...
  const tagCheckChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (checkbox.checked) {
      ...
      onSelectedTagsChange(
        selectedTags.filter((tag) => tag !== (checkbox.parentElement?.textContent ?? ''))
      );
    }
  };
  return (
    <>
          ...
          <span id="tags">
            {tags.map((tag: string) => (
              <span key={tag} className="tag">
                {tag}
                <input type="checkbox" onChange={tagCheckChange} />
              </span>
            ))}
          </span>
        </label>
      ...
      <ToggleSwitch
        switchLabel="Sort By"
        labelOff="Last Execution"
        labelOn="URL"
        checked={sortByUrl}
        onChange={onSortByUrlChange}
      />
    </>
  );
}
