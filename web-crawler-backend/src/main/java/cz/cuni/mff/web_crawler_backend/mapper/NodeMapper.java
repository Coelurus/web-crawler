...
public class NodeMapper {
    ...
    public NodeDTO mapToNodeDTO(CrawlResult result) {
        ...
        List<CrawlResult> linkedResults = crawlResultRepository.findAllById(
                links.stream().map(CrawlLink::getTo).toList()
        );
        ...
        return new NodeDTO(
                ...
                linkedResults.stream().map(this::mapToNodeDTO).toList(),
                owner.getWebsite()
        );
    }
}
